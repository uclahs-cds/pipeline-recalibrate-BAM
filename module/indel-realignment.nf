...
/*
    ...
    input:
        ...
        (bam, bam_index, interval_id, split_interval):
            tuple of input BAM and index files, interval ID, and split interval
...
process run_IndelRealigner_GATK {
    ...
    path(bundle_known_indels_vcf_gz)
    path(bundle_known_indels_vcf_gz_tbi)
    tuple path(bam), path(bam_index), val(interval_id), path(split_intervals), val(has_unmapped), path(target_intervals_RTC)

    output:
    tuple path("*${output_extension}.bam"), path("*${output_extension}.bai"), val(interval_id), path(split_intervals), val(has_unmapped), val(output_extension), emit: output_ch_indel_realignment

    script:
    arg_bam = bam.sort().collect{ "--input_file '$it'" }.join(' ')
    unmapped_interval_option = (has_unmapped) ? "--intervals unmapped" : ""
    combined_interval_options = "--intervals ${split_intervals} ${unmapped_interval_option}"
...
workflow realign_indels {
    ...
    run_RealignerTargetCreator_GATK(
        ...
        "${params.getOrDefault('input_input_intervals', null) ?: params.work_dir + '/NO_FILE.bed'}",
        input_ch_rtc
        )
